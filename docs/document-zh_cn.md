...
### 网络连接代理 (connectionProxy)

在连接至Minecraft服务器时，使用给定代理地址进行连接

该选项是一个开关，你还需在选项网络连接代理地址中配置正确的代理地址

注意：

- 你需要在连接服务器前配置好代理。在连接至服务器后对本选项的修改在下次链接时生效

- 代理仅影响客户端至服务端的连接，不影响对Mojang认证服务器的请求连接

已知问题：服务器列表界面可能会变卡，卡顿取决于连接至代理服务器的耗时
...
### 网络连接代理地址 (connectionProxyUrl)

支持协议：sock4, sock5, http

格式："协议://[用户名:密码@]主机名:端口"

例子："socks://localhost:1080", "http://foo:bar@192.168.1.1:8080"
