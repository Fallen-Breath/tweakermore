...
### 自动清空容器-忽略末影箱 (autoCleanContainerIgnoreEnderChest)

使用自动清空容器功能时，忽略末影箱

注意：这里使用的末影箱检测方式是，检查打开的容器GUI的标题和大小，

判断是否与末影箱GUI的匹配，因此可能无法做到100%的准确识别
...
### 自动装填容器-忽略末影箱 (autoFillContainerIgnoreEnderChest)
...
### 信息展示-比较器-文本缩放 (infoViewComparatorTextScale)

选项信息展示-比较器渲染的文本的缩放比例
...
### 信息展示-漏斗-文本缩放 (infoViewHopperTextScale)
...
### 信息展示-红石粉更新顺序-文本缩放 (infoViewRedstoneDustUpdateOrderTextScale)

选项信息展示-红石粉更新顺序渲染的文本的缩放比例
...
### 降低爆炸音效 (lowerExplosionSound)

降低爆炸音效的音量，使其没那么吵

另见：降低爆炸音效-数量限制, 降低爆炸音效-百分比
...
### 降低爆炸音效-数量限制 (lowerExplosionSoundLimitPerTick)

限制每个tick内在同一坐标播放的爆炸音效的数量
...
### 降低爆炸音效-百分比 (lowerExplosionSoundPercent)
...
### 将已连接服务器条目移至顶部 (moveConnectedServerEntryToTop)

在连上服务器时，把相应的服务器条目移到服务器列表的顶部
...
### 粒子数量上限 (particleLimit)

修改客户端的粒子数量上限。同时影响运算的上限和渲染的上限
